...
export class RainDocument {
     ...
     */
    public async handleImport(imp: ParsedChunk): Promise<AST.Import> {
        ...
        if (_result.hash && this.imports.find(v => v.hash === _result.hash)) _result.problems.push({
            ...
            if (!_record) this.problems.push({
                ...
                if (_metaMaps === undefined) _result.problems.push({
                    ...
                    if (_dispair) {
                        try {
                            const _bytecode = Meta.decodeMap(_dispair);
                            ...
                            if (!this._ignoreAM) {
                                ...
                                if (!_authoringMetaBytes) _authoringMetaBytes = 
                                    await this.metaStore.getAuthoringMeta(
                                        await Meta.hash([_dispair], false),
                                        true
                                    );
                                if (!_authoringMetaBytes) {
                                    ...
                                    // const _amm = Meta.decode(_authoringMetaBytes).find(v => 
                                    //     v.get(1) === Meta.MagicNumbers.AUTHORING_META_V1
                                    // );
                                    // if (!_amm) throw "";
                                    _authoringMeta = Meta.Authoring.abiDecode(
                                        _authoringMetaBytes
                                    );
                                }
                            }
                    ...
                    if (!_isCorrupt && _contractMeta) {
                        try {
                            ...
                            //                 ? "meta sequence contains invalid Meta.Contract, reason: " 
                            //                 : "invalid Meta.Contract, reason: ") + (error as Error).message,
                            //             position: _result.hashPosition,
                            ...
                            // else _result.problems.push({
                            //     msg: _record.type === "sequence" 
                            //         ? "meta sequence contains invalid Meta.Contract" 
                            //         : "invalid Meta.Contract",
                            //     position: _result.hashPosition,
                            //     code: ErrorCode.InvalidContractMeta
                            // });
                            const _parsed = Meta.Contract.get(_contractMeta);
                            _result.sequence.ctxmeta = this.toContextAlias(_parsed);
                        }
                        catch (error) {
                            // const _errorHeader = _record.type === "sequence" 
                            //     ? "meta sequence contains corrupt Meta.Contract, reason: " 
                            //     : "corrupt Meta.Contract, reason: ";
                            _result.problems.push({
                                msg: "corrupt meta",
                                position: _result.hashPosition,
                                code: ErrorCode.CorruptMeta
                    ...
                    if (!_isCorrupt && _dotrain) {
                        try {
                            const _str = String.fromCharCode(
                                ...Meta.decodeMap(_dotrain) as Uint8Array
                            );
        ...
        if (this.importDepth === 0) {
            ...
            this.bindings.forEach(v => {
                if (v.constant === undefined && v.elided === undefined) {
                    ...
                    v.problems.push(...(this._ignoreUAM
                        ? ((v.exp as any).problems as AST.Problem[]).map(e => ({
                            msg: e.msg,
                            position: [
                                e.position[0] + v.contentPosition[0],
                                e.position[1] + v.contentPosition[0]
                            ...
                            code: e.code,
                        }) as AST.Problem).filter(
                            e => e.code !== ErrorCode.UndefinedOpcode
                        )
                        : ((v.exp as any).problems as AST.Problem[]).map(e => ({
                            msg: e.msg,
                            position: [
