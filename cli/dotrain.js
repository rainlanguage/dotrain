...
const getOptions = async args => new Command("dotrain")
    .description("CLI command to compile/decompile dotrain.")
    ...
    .option("-d, --decompile <opmeta hash>", "Use decompiling mode with a specific opmeta hash, to decompile an ExpressionConfig in a .json to a .rain")
    ...
    if (options.batchCompile) {
        ...
        if (
            ...
            for (let i = 0; i < mappingContent.length; i++) {
                ...
                const result = await rainlangc(dotrainContent, mappingContent[i].expressions);
        ...
        if (options.compile && options.decompile) throw "cannot use -c and -d simultaneously!";
        else {
            if (options.compile) {
