...
pub struct Rainlang {
    pub text: String,
    pub ast: Vec<RainlangSource>,
    pub problems: Vec<Problem>,
    pub comments: Vec<Comment>,
    pub authoring_meta: AuthoringMeta,
    pub bytecode: Vec<u8>,
...
impl Rainlang {
    pub fn new(
        ...
        bytecode: Vec<u8>,
        dotrain_options: Option<DotrainOptions>
    ) -> Rainlang {
        let mut ignore_uam = false;
        let mut namespaces: Namespace = HashMap::new();
        match dotrain_options {
            Some(opts) =>  {
                if let Some(ns) = opts.namespaces {
                    namespaces = ns;
                }
                if let Some(v) = opts.ignore_authoring_meta {
                    ignore_uam = v;
                }
            }
            None => {}
        ...
        let mut rl = Rainlang {
            ...
            state: RainlangState { 
                nodes: vec![], 
                aliases: vec![], 
                parens: Parens { open: vec![], close: vec![] }, 
                depth: 0, 
                runtime_error: None 
            },
        };
    ...
    pub fn parse(&mut self) {
        match self._parse() {
            Ok(parsed) => (),
            Err(e) => {
                self.state.runtime_error = Some(e);
                self.problems.push(Problem { 
                    msg: e.to_string(), 
                    position: Offsets(0, -1), 
...
//     };
    ...
    fn _find_next_boundry(&mut self, str: String) -> i32 {
        return str[1]
        // .search(/[()<>\s]/g);
    }
    fn _reset_state(&mut self) {
        ...
        self.state.aliases = vec![];
        self.state.parens.open = vec![];
        self.state.parens.close = vec![];
    }


    fn _parse() -> anyhow::Result<()> {
